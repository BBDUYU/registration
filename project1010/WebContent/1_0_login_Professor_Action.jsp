...
<%@ page import="com.pj.dto.ProDTO" %> <!-- login 함수를 사용하여 로그인 처리를 하기 위해 user패키지의 UserDAO를 불러온다 -->
<%@ page import="java.io.PrintWriter" %> <!-- 자바 스크립트 문장을 작성하기 위해 사용-->
<% request.setCharacterEncoding("UTF-8"); %>
<jsp:useBean id="pro" class="com.pj.dto.ProDTO" scope="session"/> <!-- 한명의 회원 정보를 담는 User클래스를 자바 빈즈로 사용하며 현재 page안에서만 빈즈를 사용 -->
...
<%  // 로그인 페이지에서 넘겨준 userID와 userPassword를 받아서 로그인 판별
		...
		int result = proDAO.login(pro);
		if (result == 1){ // 로그인 정보가 맞으면 자바스크립트를 실행하여 페이지를 이동시킴
			PrintWriter script = response.getWriter();
			script.println("<script>");%>
			<jsp:forward page = "StudentServlet?command=professorInfo"/><% // 13_my_info 페이지로 사용자를 보냄 
			script.println("</script>");
		}
		else if (result == 0){ 
			PrintWriter script = response.getWriter();
